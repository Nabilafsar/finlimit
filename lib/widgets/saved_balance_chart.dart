...
class _LineChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // PERBAIKAN: handle data kosong atau semua 0
    ...
    // PERBAIKAN: support nilai negatif
    final maxVal = data.reduce((a, b) => a > b ? a : b);
    final minVal = data.reduce((a, b) => a < b ? a : b);
    final maxY = (maxVal * 1.3).clamp(0.1, double.infinity);
    final minY = minVal < 0 ? (minVal * 1.3) : 0.0;
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              interval: data.length <= 6
                  ? 1
                  : (data.length / 4).roundToDouble(),
              getTitlesWidget: (value, meta) {
                final index = value.toInt();
                if (index < 0 ||
                    index >= vm.chartLabels.length) {
