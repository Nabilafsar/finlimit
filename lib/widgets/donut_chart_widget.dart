...
class DonutChartWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      // PERBAIKAN: tampilkan empty state jika tidak ada transaksi
      child: vm.hasTransactions
          ? _DonutContent(vm: vm)
          : _EmptyDonut(),
    );
  }
...
class _DonutContent extends StatelessWidget {
  final StatisticViewModel vm;
  const _DonutContent({required this.vm});

  @override
  Widget build(BuildContext context) {
    final spend = vm.categorySpend;

    // PERBAIKAN: hanya tampilkan kategori yang punya nilai > 0
    final activeCategories = TransactionCategory.values
        .where((cat) => (spend[cat] ?? 0) > 0)
        .toList();
    ...
    return Column(
      children: [
        ...
        // Legend hanya kategori yang aktif
        Wrap(
          spacing: 20,
          runSpacing: 10,
